...
class CPU {
 ...
 public:
  ...
  void process() const {
    cout << "CPU " << model << " running at " << speed
         << " GHz is processing data..." << endl;
  }
};
...
class Computer {
 ...
 public:
  ...
  void start() const {
    cout << "Starting computer " << brand << " with " << ram << "GB RAM..."
         << endl;
    cpu.process();
  }
};
...
int main() {
  ...
  cout << "Computer object destroyed, but the CPU still exists and works:"
       << endl;
  cpu1.process();
