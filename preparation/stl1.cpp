...
int main() {
  ...
  sort(names.begin(), names.end(),
       [](string n1, string n2) { return n1 > n2; });
  for (string n : names) cout << n << endl;

  cout << "###############" << endl;
  // Ταξινόμηση κατά μήκος ονόματος
  sort(names.begin(), names.end(),
       [](string n1, string n2) { return n1.length() < n2.length(); });
  for (string n : names) cout << n << endl;

  cout << "###############" << endl;
  // Διαγραφή όλων των ονομάτων με μήκος μικρότερο από 6
  vector<string>::iterator itr = remove_if(
      names.begin(), names.end(), [](string n) { return n.length() < 6; });
