...
public:
    ebook(string title, string authors, int file_size) : title(title), authors(authors), file_size(file_size) {}
    string get_title()
    {
        return title;
    }
...
{
    ebook a[3] = {
        ebook("The Pragmatic Programmer", "Andrew Hunt and David Thomas", 1234567),
    ...
    try
    {
        for (int i = 0; i < 3; i++)
        {
            cout << a[i].get_title() << " " << a[i].get_authors() << " " << a[i].get_file_size(choice) << endl;
        }
    }
    catch (string ex)
    {
