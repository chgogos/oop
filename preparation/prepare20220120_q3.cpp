...
class document {
 ...
 public:
  ...
  void add_author(string name) { authors.push_back(name); }

  virtual string info() = 0;

  bool operator<(document &other) { return authors[0] < other.authors[0]; }
};
...
class email : public document {
 ...
 public:
  email(string a_date, string a_sender, string a_subject)
      : document(a_date), sender(a_sender), subject(a_subject) {}
  string get_sender() { return sender; }
  string get_subject() { return subject; }
  vector<string> get_recipients() { return recipients; }
  void add_recipient(string recipient) { recipients.push_back(recipient); }
  string info() {
    stringstream ss;
    ss << "Sender: " << sender;
    ss << " Subject: " << subject;
...
int main() {
  ...
  sort(v.begin(), v.end(), [](auto d1, auto d2) { return *d1 < *d2; });
